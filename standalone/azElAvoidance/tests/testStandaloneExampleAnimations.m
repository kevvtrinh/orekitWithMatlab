...
function testEveryExampleUsesUnifiedFunnel(testCase)
root = fileparts(fileparts(mfilename("fullpath")));
folder = fullfile(root, "examples");
examples = dir(fullfile(folder, "example*.m"));
for k = 1:numel(examples)
    ...
    usesFunnel = contains(source, "planAzElSpaceTimeFunnel");
    usesRunner = contains(source, "runAzElGauntletCase");
    usesInterceptWrapper = ...
        contains(source, "planAzElMovingTargetIntercept");
    verifyTrue(testCase, ...
        usesFunnel || usesRunner || usesInterceptWrapper, ...
        sprintf('%s bypasses the unified funnel.', examples(k).name));
    verifyEmpty(testCase, regexp(source, ...
        "planAzElAutonomousCorridor|planAzElAvoidance\(|" + ...
        "planAzElKinodynamicA(?:RA)?Star", ...
