...
% success. obstacleWorkspace may also be an AdaptiveAzElTimeMesh. In that
% mode, collision checks use conservative adaptive occupancy; validate the
% final path against the original packed polygons.
...
isAdaptiveMesh = isstruct(obstacleWorkspace) && ...
    isfield(obstacleWorkspace, "Format") && ...
    obstacleWorkspace.Format == "AdaptiveAzElTimeMesh";
if ~isAdaptiveMesh && (~isstruct(obstacleWorkspace) || ...
        ~isfield(obstacleWorkspace, "Format") || ...
        obstacleWorkspace.Format ~= "AzElTimeObstacleWorkspace")
    obstacleWorkspace = buildAzElTimeObstacleWorkspace(obstacleWorkspace);
end
validateCollisionModel(obstacleWorkspace);
...
if goalIndex == 0
    ...
    result.CollisionModelFormat = obstacleWorkspace.Format;
    return;
end
...
querySeconds = repmat(querySeconds, 1, size(accelerations, 1));
    blocked = queryCollisionModel(workspace, ...
        azimuth(:), elevation(:), querySeconds(:), options);
...
function collision = pointCollision( ...
    ...
    blocked = queryCollisionModel(workspace, ...
        startState(1), startState(2), startTimeSeconds, options);
    collision = any(blocked);
end

function blocked = queryCollisionModel( ...
        model, azimuth, elevation, querySeconds, options)
if model.Format == "AdaptiveAzElTimeMesh"
    blocked = queryAdaptiveAzElTimeMesh( ...
        model, azimuth, elevation, querySeconds);
else
    blocked = queryAzElTimeObstacle(model, azimuth, elevation, ...
        querySeconds, struct( ...
...
function validateCollisionModel(model)
isPacked = isstruct(model) && isfield(model, "Format") && ...
    model.Format == "AzElTimeObstacleWorkspace" && ...
    isfield(model, "ReferenceTime") && isfield(model, "Obstacles");
isAdaptive = isstruct(model) && isfield(model, "Format") && ...
    model.Format == "AdaptiveAzElTimeMesh" && ...
    isfield(model, "ReferenceTime") && isfield(model, "Leaves") && ...
    isfield(model, "TimeSeconds");
if ~isPacked && ~isAdaptive
    error("planAzElKinodynamicAStar:InvalidWorkspace", ...
        ["Use buildAzElTimeObstacleWorkspace or " ...
        "buildAdaptiveAzElTimeMesh to create the collision model."]);
end
end
