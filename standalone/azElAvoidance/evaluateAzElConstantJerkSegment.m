...
jerkRows = [4 10 16];
phaseJerk_deg_s3 = law(jerkRows, :);
if max(abs(phaseJerk_deg_s3 - phaseJerk_deg_s3(1, :)), [], "all") <= ...
        64 * eps(max(1, max(abs(phaseJerk_deg_s3), [], "all")))
    duration_s = law(18, 1) + law(17, 1);
    extrema = constantJerkExtrema( ...
        law(1, :), law(2, :), law(3, :), ...
        phaseJerk_deg_s3(1, :), duration_s);
    return;
end
...
function extrema = constantJerkExtrema( ...
...
for axisIndex = 1:2
    velocityTimes_s = zeros(3, 1);
    velocityTimes_s(1:2) = [0; duration_s];
    velocityTimeCount = 2;
    if abs(jerk_deg_s3(axisIndex)) > ...
            eps(max(1, abs(acceleration0_deg_s2(axisIndex))))
        accelerationRoot_s = -acceleration0_deg_s2(axisIndex) / ...
            jerk_deg_s3(axisIndex);
        if accelerationRoot_s > 0 && accelerationRoot_s < duration_s
            velocityTimeCount = 3;
            velocityTimes_s(3) = accelerationRoot_s;
        end
    end
    velocityTimes_s = velocityTimes_s(1:velocityTimeCount);
    velocityValues_deg_s = velocity0_deg_s(axisIndex) + ...
        acceleration0_deg_s2(axisIndex) * velocityTimes_s + ...
        0.5 * jerk_deg_s3(axisIndex) * velocityTimes_s.^2;
    maximumVelocity_deg_s(axisIndex) = max(abs(velocityValues_deg_s));
    positionTimes_s = [0; duration_s; velocityRoots( ...
        velocity0_deg_s(axisIndex), acceleration0_deg_s2(axisIndex), ...
        jerk_deg_s3(axisIndex), duration_s)];
    positionValues_deg = position0_deg(axisIndex) + ...
        velocity0_deg_s(axisIndex) * positionTimes_s + ...
        0.5 * acceleration0_deg_s2(axisIndex) * positionTimes_s.^2 + ...
        (1 / 6) * jerk_deg_s3(axisIndex) * positionTimes_s.^3;
    minimumPosition_deg(axisIndex) = min(positionValues_deg);
    maximumPosition_deg(axisIndex) = max(positionValues_deg);
end
finalAcceleration_deg_s2 = acceleration0_deg_s2 + ...
    jerk_deg_s3 * duration_s;
extrema = struct( ...
    "minimumPosition_deg", minimumPosition_deg, ...
    "maximumPosition_deg", maximumPosition_deg, ...
    ...
    acceleration0_deg_s2; finalAcceleration_deg_s2]), [], 1), ...
    "maximumJerk_deg_s3", abs(jerk_deg_s3));
end
