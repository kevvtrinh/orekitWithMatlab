...
plannerTimer = tic;
% A short joint-search probe belongs to the requested motion policy, but a
% documented fallback must still receive the full budget the caller gave it.
% This credit is added only to remaining-budget checks; reported elapsed time
% continues to include every attempted search.
preliminaryJointTimeCredit_s = 0;
...
positiveOptionFields = ["SampleTime_s", "ValidationStep_s", ...
    ...
    "JointKinematicAttemptTime_s", ...
    "ParetoRefinementNodeStep_s", ...
    "ParetoRefinementMinimumTimeScale"];
...
positiveIntegerOptionFields = ["MaximumSafeIntervalSamples", ...
    ...
    "ParetoRefinementTimeScaleCount", ...
    "ParetoRefinementMaximumKnotCount", ...
    "ParetoRefinementCoordinatePassCount", ...
    "ParetoRefinementCandidateBudget"];
...
validateattributes(options.ParetoRefinementBlendSchedule, {'numeric'}, ...
    {'vector', 'real', 'finite', '>=', 0, '<=', 1});
options.ParetoRefinementBlendSchedule = unique( ...
    double(options.ParetoRefinementBlendSchedule(:).'), "stable");
validateattributes(options.ParetoRefinementCoordinateMoveFractions, ...
    {'numeric'}, {'vector', 'real', 'finite', '>', 0, '<=', 1});
options.ParetoRefinementCoordinateMoveFractions = unique( ...
    double(options.ParetoRefinementCoordinateMoveFractions(:).'), ...
    "stable");
if options.ParetoRefinementMinimumTimeScale > 1
    error("planAzElDijkstra:InvalidParetoMinimumTimeScale", ...
        "ParetoRefinementMinimumTimeScale must not exceed one.");
end
if options.ParetoRefinementMaximumKnotCount < 3
    error("planAzElDijkstra:InvalidParetoKnotCount", ...
        "ParetoRefinementMaximumKnotCount must be at least three.");
end
...
% accelerations, time, and bounded-jerk actions together.
% "jointParetoKinematic" runs that joint search and then continuously tests
% shorter/faster blends. When the joint lattice cannot finish, its explicit
% profile fallback still receives the same continuous Pareto pass. Every
% experimental result is densely checked against every moving obstacle.
...
elseif any(motionModeName == ["jointparetokinematic", ...
        "jointboundedjerkpareto", "jointpareto", ...
        "boundedjerkpareto"])
    options.MotionMode = "jointParetoKinematic";
else
    error("planAzElDijkstra:InvalidMotionMode", ...
        ["MotionMode must be profile, pathFirstThenKinematic, " ...
        "pathStateSpaceKinematic, jointStateSpaceKinematic, or " ...
...
jointParetoModeRequested = options.MotionMode == "jointParetoKinematic";
pathFirstModeRequested = any(options.MotionMode == [ ...
    "pathFirstThenKinematic", "pathStateSpaceKinematic", ...
    "jointParetoKinematic"]);
stateSpaceModeRequested = ...
    options.MotionMode == "pathStateSpaceKinematic";
jointStateSpaceModeRequested = any(options.MotionMode == [ ...
    "jointStateSpaceKinematic", "jointParetoKinematic"]);
...
motionPlanning = struct( ...
    ...
    "ParetoRefinementAttempted", false, ...
    "ParetoRefinementSucceeded", false, ...
    "ParetoRefinementImproved", false, ...
    "ParetoRefinementMessage", "", ...
    "ParetoRefinement", struct(), ...
    "PhysicalVerificationAttempted", false, ...
    "PhysicalVerificationSucceeded", false, ...
    "PhysicalVerificationMessage", "", ...
    "PhysicalVerification", struct());
if pathFirstModeRequested && ~pathFirstModeIsCompatible
...
if jointStateSpaceModeRequested
    ...
    if jointParetoModeRequested && options.FallbackToProfile
        jointPhaseBudget_s = min( ...
            options.JointKinematicAttemptTime_s, options.MaxSearchTime_s);
    elseif options.FallbackToProfile
    ...
    if jointSearch.Success
        ...
        if jointParetoModeRequested
            [plan, paretoReceipt] = refineAzElPlanPareto( ...
                plan, obstacleField, initialState, goalState, ...
                limits, options);
            motionPlanning.ParetoRefinementAttempted = true;
            motionPlanning.ParetoRefinementSucceeded = ...
                paretoReceipt.Success;
            motionPlanning.ParetoRefinementImproved = ...
                paretoReceipt.Improved;
            motionPlanning.ParetoRefinementMessage = ...
                paretoReceipt.Message;
            motionPlanning.ParetoRefinement = paretoReceipt;
            motionPlanning.PhysicalVerificationAttempted = true;
            motionPlanning.PhysicalVerificationSucceeded = ...
                paretoReceipt.PhysicalVerificationPassed;
            motionPlanning.PhysicalVerificationMessage = ...
                paretoReceipt.PhysicalVerification.Message;
            motionPlanning.PhysicalVerification = ...
                paretoReceipt.PhysicalVerification;
            plan.motionPlanning = motionPlanning;
            plan.retiming.ParetoRefinement = paretoReceipt;
        end
    ...
    if jointParetoModeRequested
        preliminaryJointTimeCredit_s = toc(plannerTimer);
    end
end
...
if shouldAttemptPathFirst
    ...
    for staticGridLevel = 1:numel(gridStepSchedule_deg)
        remainingStaticTime_s = options.MaxSearchTime_s + ...
            preliminaryJointTimeCredit_s - toc(plannerTimer);
        if pathFirstModeRequested && options.FallbackToProfile
    ...
    if ~isempty(fieldnames(bestStaticPlan))
        ...
        if jointParetoModeRequested && motionPlanning.FallbackUsed && ...
                bestStaticPlan.retiming.MotionStyle == ...
                    "continuousCornerBlend"
            % Static fallback still uses the smooth minimum-time path clock;
            % name that actual command instead of the historical profile.
            motionPlanning.SelectedMode = "pathFirstThenKinematic";
        elseif jointStateSpaceModeRequested && motionPlanning.FallbackUsed
        ...
        if jointParetoModeRequested
            [bestStaticPlan, paretoReceipt] = refineAzElPlanPareto( ...
                bestStaticPlan, obstacleField, initialState, goalState, ...
                limits, options);
            motionPlanning.ParetoRefinementAttempted = true;
            motionPlanning.ParetoRefinementSucceeded = ...
                paretoReceipt.Success;
            ...
            motionPlanning.PhysicalVerification = ...
                paretoReceipt.PhysicalVerification;
            bestStaticPlan.motionPlanning = motionPlanning;
            bestStaticPlan.retiming.ParetoRefinement = paretoReceipt;
        end
...
for gridLevelIndex = 1:numel(gridStepSchedule_deg)
    remainingSearchTime_s = options.MaxSearchTime_s + ...
        preliminaryJointTimeCredit_s - toc(plannerTimer);
...
if jointParetoModeRequested
    ...
    motionPlanning.ParetoRefinementAttempted = true;
    motionPlanning.ParetoRefinementSucceeded = paretoReceipt.Success;
    motionPlanning.ParetoRefinementImproved = paretoReceipt.Improved;
    motionPlanning.ParetoRefinementMessage = paretoReceipt.Message;
    motionPlanning.ParetoRefinement = paretoReceipt;
    motionPlanning.PhysicalVerificationAttempted = true;
    motionPlanning.PhysicalVerificationSucceeded = ...
        paretoReceipt.PhysicalVerificationPassed;
...
if any(options.MotionMode == [ ...
        "pathFirstThenKinematic", "jointParetoKinematic"])
    ...
    if ~retimed.Success && ...
            options.MotionMode == "jointParetoKinematic" && ...
            options.FallbackToProfile
        % The topology is already exact-checked. If a tight corner or wrap
        % seam cannot support one continuous clock, reuse that safe route
        % with the established segment profile instead of discarding it and
        % starting a much larger time-aware search from scratch.
        continuousRetimingMessage = retimed.Message;
        retimed = retimeStaticRouteWithSegmentStops( ...
            obstacleField, initialState, goalState, ...
            routePositions_deg, axisLimits, options);
        if retimed.Success
            retimed.Message = "Continuous path-first retiming failed: " + ...
                continuousRetimingMessage + ...
                " The validated static route used profile fallback.";
        end
    end
...
% The clock search samples normalized progress uniformly. A long, tightly
% curved route can place its sharpest PCHIP curvature between those samples.
% Recheck on the actual collision grid and stretch the clock by the exact
% measured ratio. Velocity falls with 1/T and acceleration with 1/T^2, so
% this correction is deterministic and does not change the spatial path.
for limitCorrectionIndex = 1:8
    validationProfile = makeContinuousPathProfile(pathModel, ...
        initialState.time_s, goalState.time_s, timeScaling, ...
        collisionSampleStep_s, axisLimits, options);
    velocityRatio = max(abs(validationProfile.velocity_deg_s) ./ ...
        axisLimits.maxVelocity_deg_s, [], "all");
    accelerationRatio = max(abs( ...
        validationProfile.acceleration_deg_s2) ./ ...
        axisLimits.maxAcceleration_deg_s2, [], "all");
    requiredDurationScale = max(velocityRatio, sqrt(accelerationRatio));
    if requiredDurationScale <= 1 + 1e-9
        break
    end
    timeScaling.Duration_s = timeScaling.Duration_s * ...
        requiredDurationScale * 1.001;
    minimumManeuverTime_s = timeScaling.Duration_s;
    if minimumManeuverTime_s > availableManeuverTime_s + 1e-9
        break
    end
end
% The last loop pass may have stretched the duration. Rebuild once more so
% the acceptance check always examines that final clock, not the prior one.
validationProfile = makeContinuousPathProfile(pathModel, ...
    initialState.time_s, goalState.time_s, timeScaling, ...
    collisionSampleStep_s, axisLimits, options);
...
options = struct( ...
    ...
    "ParetoRefinementNodeStep_s", 0.5, ...
    "ParetoRefinementBlendSchedule", ...
        [1 0.75 0.5 0.25 0.125 0.0625], ...
    "ParetoRefinementTimeScaleCount", 9, ...
    "ParetoRefinementMinimumTimeScale", 0.25, ...
    "ParetoRefinementMaximumKnotCount", 401, ...
    "ParetoRefinementCoordinatePassCount", 2, ...
    "ParetoRefinementCandidateBudget", 48, ...
    "ParetoRefinementCoordinateMoveFractions", [0.25 0.5], ...
