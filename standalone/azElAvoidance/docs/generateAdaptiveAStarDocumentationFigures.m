function files = generateAdaptiveAStarDocumentationFigures()
...
ax = nexttile(layout);
hold(ax, "on");
obstacle = workspace.Obstacles(1);
...
startState = restState(0, [-8 0]);
stopState = restState(30, [8 0]);
...
for k = 1:numel(attempts)
    ...
    blocked = queryAzElTimeObstacle(workspace, ...
        azimuthGrid(:), elevationGrid(:), ...
        repmat(startState.time_s, numel(azimuthGrid), 1), struct( ...
    ...
    plot(ax, startState.position_deg(1), ...
        startState.position_deg(2), "s", ...
        "MarkerFaceColor", [0.12 0.65 0.25], ...
        "MarkerEdgeColor", "k", "DisplayName", "Start");
    plot(ax, stopState.position_deg(1), ...
        stopState.position_deg(2), "d", ...
...
title(layout, ...
    "Static mode: progressively finer A* graphs, exact polygon checks", ...
    "FontWeight", "bold");
exportgraphics(figureHandle, file, "Resolution", 180);
close(figureHandle);
end
...
startState = restState(0, [-6 0]);
stopState = restState(30, [6 0]);
...
for k = 1:numel(probeElevation)
    blocked(k, :) = queryAzElTimeObstacle(workspace, ...
