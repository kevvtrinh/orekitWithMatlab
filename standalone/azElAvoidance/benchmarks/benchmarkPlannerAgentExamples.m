...
% from distorting planner timing. The default 180-second envelope includes
% the long-horizon spinning-rod profile and its reserved fallback.
...
if any(successfulComparison)
    runtimeRatio = agentSearch_s(successfulComparison) ./ ...
        baselineSearch_s(successfulComparison);
    pathRatio = agentPath_deg(successfulComparison) ./ ...
        baselinePath_deg(successfulComparison);
    medianRuntimeRatio = median( ...
        runtimeRatio);
    medianPathRatio = median(pathRatio);
    maximumPathRatio = max(pathRatio);
    maximumCompletionDelay_s = max( ...
        agentCompletion_s(successfulComparison) - ...
        baselineCompletion_s(successfulComparison));
else
    medianRuntimeRatio = Inf;
    medianPathRatio = Inf;
    maximumPathRatio = Inf;
    maximumCompletionDelay_s = Inf;
end
report = struct( ...
    "results", results, ...
    ...
    "maximumPathRatioOnMutualSuccess", maximumPathRatio, ...
    "maximumCompletionDelay_s", maximumCompletionDelay_s, ...
    "agentBudget_s", agentBudget_s);
fprintf( ...
    "\nAgent summary: %d/%d success (%.1f%%), " + ...
    "median runtime ratio %.3f, median path ratio %.3f, " + ...
    "maximum path ratio %.3f.\n", ...
    ...
    report.maximumPathRatioOnMutualSuccess);
end
