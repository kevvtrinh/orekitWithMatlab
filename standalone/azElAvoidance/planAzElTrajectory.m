...
primitiveBounds_deg = zeros(primitiveCount, 4);
for primitiveIndex = 1:primitiveCount
    ...
    primitiveBounds_deg(primitiveIndex, :) = [ ...
...
[reverseCostToGo, reverseDiagnostics, reverseBlockedCell] = ...
...
useDenseStateLookup = denseLookupEntryCount <= ...
    options.MaximumDenseStateLookupEntries;
...
stateNextSameCell = zeros(maximumStoredStates, 1, "uint32");
cellStateHead = zeros(gridCellCount, 1, "uint32");
stateCellId(1) = uint32(startCellId);
stateTimeIndex(1) = uint32(0);
stateCost(1) = 0;
stateCount = 1;
initialStateIsStationary = all(abs(initialState.velocity_deg_s) <= ...
    options.KinematicTolerance) && all(abs( ...
    initialState.acceleration_deg_s2) <= options.KinematicTolerance);
staticDominanceEnabled = staticEnvironmentTimeInvariant;
if staticDominanceEnabled && initialStateIsStationary
    cellStateHead(startCellId) = uint32(1);
end
...
while heapCount > 0
    ...
    for transitionIndex = 1:transitionCount
        ...
        if reverseBlockedCell(nextCellId)
        ...
        canUseCachedPrimitive = transitionDistance_deg > 0 && ...
            (currentStateIndex ~= 1 || ...
            ~initialStateHasNonzeroDerivatives);
        ...
        if canUseCachedPrimitive && ~isempty(obstacleModel) && ...
                ~staticEnvironmentTimeInvariant
            relativeBounds_deg = primitiveBounds_deg(transitionIndex, :);
            transitionBounds_deg = relativeBounds_deg + [ ...
                currentPosition_deg(1), currentPosition_deg(1), ...
                currentPosition_deg(2), currentPosition_deg(2)];
        ...
        elseif collisionCacheEligible
            ...
            if transitionDistance_deg > 0
                if canUseCachedPrimitive
                    ...
                    transitionLaws = compositeConstantJerkLaws( ...
                        currentPosition_deg, currentVelocity_deg_s, ...
                        currentAcceleration_deg_s2, nextPosition_deg, ...
                        [0 0], [0 0], duration_s);
                    kinematicsPass = compositeKinematicsPass( ...
                        transitionLaws, duration_s, maximumVelocity_deg_s, ...
                        maximumAcceleration_deg_s2, maximumJerk_deg_s3, ...
                        options.KinematicTolerance);
                    transitionExtrema = repmat(struct(), 0, 1);
                end
            else
                transitionLaws = constantJerkLaw( ...
                    currentPosition_deg, currentVelocity_deg_s, ...
                    currentAcceleration_deg_s2, nextPosition_deg, ...
                    [0 0], [0 0], duration_s);
                holdExtrema = evaluateAzElConstantJerkSegment( ...
                    transitionLaws, [0; duration_s]);
                transitionExtrema = holdExtrema.extrema;
                kinematicsPass = true;
            end
            if ~kinematicsPass
        ...
        if staticDominanceEnabled
            existingStateIndex = cellStateHead(nextCellId);
            nextStateIsDominated = false;
            while existingStateIndex ~= 0
                counters.dominanceComparisons = ...
                    counters.dominanceComparisons + 1;
                existingIndex = double(existingStateIndex);
                if ~stateDominated(existingIndex)
                    existingTimeIndex = double( ...
                        stateTimeIndex(existingIndex));
                    existingCost = stateCost(existingIndex);
                    if existingTimeIndex <= nextTimeIndex
                        if objective == "minimumTime"
                            reproductionCost = existingCost + ...
                                (nextTimeIndex - existingTimeIndex) * ...
                                options.TimeStep_s;
                        else
                            reproductionCost = existingCost;
                        end
                        if reproductionCost <= ...
                                nextCost + options.CostTolerance
                            nextStateIsDominated = true;
                            break;
                        end
                    end
                    if nextTimeIndex <= existingTimeIndex
                        if objective == "minimumTime"
                            reproductionCost = nextCost + ...
                                (existingTimeIndex - nextTimeIndex) * ...
                                options.TimeStep_s;
                        else
                            reproductionCost = nextCost;
                        end
                        if reproductionCost <= ...
                                existingCost + options.CostTolerance
                            stateDominated(existingIndex) = true;
                            counters.dominanceRetirements = ...
                                counters.dominanceRetirements + 1;
                        end
                    end
                end
                existingStateIndex = stateNextSameCell(existingIndex);
            end
            if nextStateIsDominated
        ...
        statePathHasWait(stateCount) = ...
            ...
            transitionDistance_deg == 0;
        if staticDominanceEnabled
            stateNextSameCell(stateCount) = cellStateHead(nextCellId);
            cellStateHead(nextCellId) = uint32(stateCount);
        end
        if useDenseStateLookup
...
if isfinite(relaxedGraphBound)
    heuristic = max(straightLineBound, relaxedGraphBound);
else
...
heapTie = zeros(heapCapacity, 1);
heapCount = 0;
heapPushes = 0;
for seedIndex = 1:numel(goalCellIds)
    seedCellId = goalCellIds(seedIndex);
    costToGo(seedCellId) = 0;
    [heapCell, heapPriority, heapTie, heapCount] = heapPush( ...
        heapCell, heapPriority, heapTie, heapCount, ...
        uint32(seedCellId), 0, 0);
...
while heapCount > 0
    ...
    [heapCell, heapPriority, heapTie, heapCount, poppedCell] = ...
        heapPop(heapCell, heapPriority, heapTie, heapCount);
    heapPops = heapPops + 1;
    ...
    for relaxedEdgeIndex = 1:size(relaxedOffsets, 1)
        relaxationAttempts = relaxationAttempts + 1;
        predecessorAzimuthIndex = ...
            azimuthIndex - relaxedOffsets(relaxedEdgeIndex, 1);
        predecessorElevationIndex = ...
            elevationIndex - relaxedOffsets(relaxedEdgeIndex, 2);
        if allowWrap
        ...
        trialCost = poppedPriority + relaxedEdgeCost(relaxedEdgeIndex);
        ...
        [heapCell, heapPriority, heapTie, heapCount] = heapPush( ...
            heapCell, heapPriority, heapTie, heapCount, ...
            uint32(predecessorCellId), trialCost, 0);
        heapPushes = heapPushes + 1;
    end
end
diagnostics = struct( ...
