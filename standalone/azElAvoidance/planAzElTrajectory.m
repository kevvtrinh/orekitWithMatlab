...
latticeCollisionStep_s = conservativeCollisionStep( ...
...
positionResolution_deg = gridStep_deg / 6;
positionBinOrigin_deg = [ ...
    unwrappedAzimuthLimits_deg(1), elevationLimits_deg(1)];
positionBinCount = ceil([ ...
    diff(unwrappedAzimuthLimits_deg), diff(elevationLimits_deg)] ./ ...
    positionResolution_deg) + 1;
finePositionCellCount = prod(positionBinCount);

%% Section 3: Compute The Reverse-Dijkstra Cost Field
[reverseCostToGo, reverseDiagnostics, ~, ...
    reverseStaticEdgeCollisionStatus] = ...
    ...
    safetyMargin_deg, allowAzimuthWrap, ...
    options.GoalConnectionRadius_deg, ...
    options.MaximumStaticReverseEdgeChecks);
projectedStaticEdgeCollisionStatus = ...
    reverseStaticEdgeCollisionStatus;
if staticEnvironmentTimeInvariant && ~isempty(obstacleModel)
    staticEdgeCollisionStatus = reverseStaticEdgeCollisionStatus;
else
...
[dynamicRelaxedCostToGo, dynamicRelaxationDiagnostics] = ...
    ...
    primitiveOffsets, primitiveDurationSteps, primitiveDistance_deg);
...
dynamicMotionFitsMacroGraph = dynamicObstacleMotionWithinGridStep( ...
    obstacleModel, gridStep_deg);
stationaryMacroGraphEnabled = ...
    (geometryIsStatic || dynamicMotionFitsMacroGraph || ...
    ...
    all(abs(terminalCandidates(1).acceleration_deg_s2) <= ...
        options.KinematicTolerance);
forwardProjectionUsesMacroStencil = stationaryMacroGraphEnabled;
reverseFieldCanBeCombined = forwardProjectionUsesMacroStencil;
...
counters = struct( ...
    ...
    "stationaryMacroGraphEnabled", stationaryMacroGraphEnabled, ...
    "useDenseStateLookup", useDenseStateLookup, ...
    "staticRestDominanceEnabled", staticRestDominanceEnabled, ...
    ...
    "cacheHits", 0, ...
    "cacheMisses", 0, ...
    "projectedStaticCacheHits", 0, ...
    "projectedStaticRejections", 0, ...
...
if useDenseStateLookup
    ...
    startKey = kinodynamicStateKey( ...
        ...
        initialState.acceleration_deg_s2, 0, ...
        positionBinOrigin_deg, positionResolution_deg, ...
        positionBinCount, finePositionCellCount, ...
...
while heapCount > 0
    ...
    if useDenseStateLookup
        ...
        currentKey = kinodynamicStateKey( ...
            statePosition_deg(currentStateIndex, :), ...
            stateVelocity_deg_s(currentStateIndex, :), ...
    ...
    for candidateIndex = 1:numel(terminalCandidates)
        ...
        if ~isempty(staticObstacleModel) && ...
                ~staticLineCollisionFree( ...
                staticObstacleModel, currentPosition_deg, ...
                candidate.positionUnwrapped_deg, safetyMargin_deg, ...
                allowAzimuthWrap)
            counters.terminalConnectorRejections = ...
                counters.terminalConnectorRejections + 1;
            counters.terminalStaticRejections = ...
                counters.terminalStaticRejections + 1;
            continue;
        end
    ...
    macroTransitionCount = primitiveCount * double( ...
        ...
        (geometryIsStatic || stationaryMacroGraphEnabled));
    ...
    if macroTransitionCount > 0 && currentIsStationary && ...
            currentIsGridAligned && stationaryMacroGraphEnabled
    ...
    for transitionIndex = 1:transitionCount
        ...
        if transitionIsMacro
            primitiveIndex = transitionIndex;
            offset = primitiveOffsets(primitiveIndex, :);
            ...
            durationSteps = primitiveDurationSteps(primitiveIndex);
            transitionDistance_deg = primitiveDistance_deg(primitiveIndex);
            transitionLaws = primitiveLaws{primitiveIndex};
            transitionExtrema = primitiveExtrema{primitiveIndex};
        ...
        if useDenseStateLookup
            ...
            nextKey = kinodynamicStateKey( ...
                ...
                positionBinOrigin_deg, ...
                positionResolution_deg, positionBinCount, ...
                finePositionCellCount, velocityResolution_deg_s, ...
                accelerationResolution_deg_s2, ...
                maximumVelocity_deg_s, maximumAcceleration_deg_s2, ...
                velocityBinCount, accelerationBinCount);
        ...
        transitionObstacleModel = obstacleModel;
        if transitionIsMacro && ...
                ~isempty(projectedStaticEdgeCollisionStatus)
            projectedStaticStatus = projectedStaticEdgeCollisionStatus( ...
                currentCellId, primitiveIndex);
            if projectedStaticStatus < 0
                counters.collisionRejections = ...
                    counters.collisionRejections + 1;
                counters.projectedStaticRejections = ...
                    counters.projectedStaticRejections + 1;
                continue;
            elseif projectedStaticStatus > 0
                transitionObstacleModel = obstacleModel( ...
                    ~staticObstacleIsAdmissible);
                counters.projectedStaticCacheHits = ...
                    counters.projectedStaticCacheHits + 1;
            end
        end
        ...
        collisionStatusWasCached = collisionCacheEligible && ...
            ...
            currentCellId, primitiveIndex) ~= 0;
        transitionMissesObstacleBounds = false;
        if ~isempty(transitionObstacleModel) && ...
                ~staticEnvironmentTimeInvariant
            ...
            transitionObstacleIsRelevant = relevantObstacleMask( ...
                transitionObstacleModel, transitionBounds_deg, ...
                currentTime_s, nextTime_s, safetyMargin_deg, ...
                allowAzimuthWrap);
            transitionObstacleModel = ...
                transitionObstacleModel(transitionObstacleIsRelevant);
            transitionMissesObstacleBounds = ...
                isempty(transitionObstacleModel);
        end
        if collisionStatusWasCached
            collisionFree = staticEdgeCollisionStatus( ...
                currentCellId, primitiveIndex) > 0;
            counters.cacheHits = counters.cacheHits + 1;
        elseif isempty(transitionObstacleModel) && canUseCachedPrimitive
        ...
        elseif collisionCacheEligible
            ...
            collisionFree = staticLineCollisionFree( ...
                transitionObstacleModel, currentPosition_deg, ...
                nextPosition_deg, ...
                safetyMargin_deg, allowAzimuthWrap);
            ...
            if collisionFree
                staticEdgeCollisionStatus( ...
                    currentCellId, primitiveIndex) = int8(1);
            else
                staticEdgeCollisionStatus( ...
                    currentCellId, primitiveIndex) = int8(-1);
            end
            ...
            for segmentIndex = 1:transitionSegmentCount
                ...
                [segmentIsFree, pointQueryCount] = segmentCollisionFree( ...
                    ...
                    latticeCollisionStep_s, safetyMargin_deg, ...
...
for chainIndex = 1:numel(parentChain) - 1
    ...
    elseif parentEdgeKind == 3
        ...
        edgeJerk_deg_s3 = (secondAcceleration_deg_s2 - ...
            firstAcceleration_deg_s2) / edgeDuration_s;
        edgeLaws = packConstantControlLaw( ...
            firstPosition_deg, firstVelocity_deg_s, ...
            firstAcceleration_deg_s2, edgeJerk_deg_s3, ...
            edgeDuration_s);
    end
    edgeIsHold = all(abs( ...
...
options = struct( ...
    ...
    "MaximumStaticReverseEdgeChecks", 2000000, ...
...
positiveScalarNames = ["TimeStep_s" "SampleTime_s" ...
    ...
    "MaximumStaticReverseEdgeChecks" ...
...
function fits = dynamicObstacleMotionWithinGridStep( ...
        obstacleModel, gridStep_deg)
fits = true;
for obstacleIndex = 1:numel(obstacleModel)
    ...
    for sliceIndex = 1:numel(obstacle.slices) - 1
        firstSlice = obstacle.slices{sliceIndex};
        secondSlice = obstacle.slices{sliceIndex + 1};
        if numel(firstSlice.regionAzimuth) ~= ...
                numel(secondSlice.regionAzimuth)
            fits = false;
            return;
        end
        for regionIndex = 1:numel(firstSlice.regionAzimuth)
            firstCenter_deg = [mean(firstSlice.regionAzimuth{regionIndex}), ...
                mean(firstSlice.regionElevation{regionIndex})];
            secondCenter_deg = [mean(secondSlice.regionAzimuth{regionIndex}), ...
                mean(secondSlice.regionElevation{regionIndex})];
            if hypot(secondCenter_deg(1) - firstCenter_deg(1), ...
                    secondCenter_deg(2) - firstCenter_deg(2)) > ...
                    gridStep_deg + 1e-12
                fits = false;
                return;
            end
...
function key = kinodynamicStateKey( ...
        position_deg, velocity_deg_s, acceleration_deg_s2, timeIndex, ...
        positionBinOrigin_deg, positionResolution_deg, ...
        positionBinCount, finePositionCellCount, velocityResolution_deg_s, ...
        accelerationResolution_deg_s2, maximumVelocity_deg_s, ...
        maximumAcceleration_deg_s2, velocityBinCount, ...
        accelerationBinCount)
positionBin = round((position_deg - positionBinOrigin_deg) ./ ...
    positionResolution_deg);
positionBin = min(positionBinCount - 1, max(0, positionBin));
...
finePositionId = positionBin(1) + 1 + ...
    positionBinCount(1) * positionBin(2);
key = uint64(finePositionId);
multiplier = uint64(finePositionCellCount);
...
assert(all(positionBin >= 0 & positionBin < positionBinCount));
end
...
timeTolerance_s = 64 * eps(max([1, abs(firstTime_s), ...
    abs(secondTime_s)]));
firstSubdivisionIndex = ceil( ...
    (firstTime_s + timeTolerance_s) / maximumStep_s);
lastSubdivisionIndex = floor( ...
    (secondTime_s - timeTolerance_s) / maximumStep_s);
if firstSubdivisionIndex <= lastSubdivisionIndex
    alignedGlobalTime_s = ...
        (firstSubdivisionIndex:lastSubdivisionIndex).' * maximumStep_s;
    uniformLocalTime_s = [0; ...
        alignedGlobalTime_s - firstTime_s; duration_s];
else
    uniformLocalTime_s = [0; duration_s];
end
...
function collisionFree = staticLinesCollisionFree( ...
        obstacleModel, firstPoint_deg, secondPoint_deg, ...
        safetyMargin_deg, allowWrap)
lineCount = size(firstPoint_deg, 1);
collisionFree = true(lineCount, 1);
if lineCount == 0 || isempty(obstacleModel)
    return;
end
lineBounds_deg = [ ...
    min(firstPoint_deg(:, 1), secondPoint_deg(:, 1)), ...
...
for obstacleIndex = 1:numel(obstacleModel)
    ...
    for regionIndex = 1:numel(slice.regionAzimuth)
        for shift_deg = azimuthShifts_deg
            polygonBounds_deg = slice.bounds_deg(regionIndex, :) + ...
                [shift_deg shift_deg 0 0];
            relevantLine = collisionFree & ...
                lineBounds_deg(:, 2) >= ...
                    polygonBounds_deg(1) - safetyMargin_deg & ...
                lineBounds_deg(:, 1) <= ...
                    polygonBounds_deg(2) + safetyMargin_deg & ...
                lineBounds_deg(:, 4) >= ...
                    polygonBounds_deg(3) - safetyMargin_deg & ...
                lineBounds_deg(:, 3) <= ...
                    polygonBounds_deg(4) + safetyMargin_deg;
            relevantLineId = find(relevantLine);
            if isempty(relevantLineId)
                continue;
            end
            edgeFirst_deg = slice.edgeFirst_deg{regionIndex};
            edgeSecond_deg = slice.edgeSecond_deg{regionIndex};
            edgeFirst_deg(:, 1) = edgeFirst_deg(:, 1) + shift_deg;
            edgeSecond_deg(:, 1) = edgeSecond_deg(:, 1) + shift_deg;
            edgeCount = size(edgeFirst_deg, 1);
            maximumPairCount = 250000;
            batchSize = max(32, min(2048, floor( ...
                maximumPairCount / max(edgeCount, 1))));
            for batchStart = 1:batchSize:numel(relevantLineId)
                batchEnd = min(numel(relevantLineId), ...
                    batchStart + batchSize - 1);
                batchId = relevantLineId(batchStart:batchEnd);
                blocked = linesBlockedByEdgesBatch( ...
                    firstPoint_deg(batchId, :), ...
                    secondPoint_deg(batchId, :), ...
                    edgeFirst_deg, edgeSecond_deg, safetyMargin_deg);
                collisionFree(batchId(blocked)) = false;
            end
        end
    end
end
...
function blocked = linesBlockedByEdgesBatch( ...
        firstPoint_deg, secondPoint_deg, edgeFirst_deg, ...
        edgeSecond_deg, margin_deg)
firstAzimuth_deg = firstPoint_deg(:, 1);
firstElevation_deg = firstPoint_deg(:, 2);
secondAzimuth_deg = secondPoint_deg(:, 1);
secondElevation_deg = secondPoint_deg(:, 2);
edgeFirstAzimuth_deg = edgeFirst_deg(:, 1).';
edgeFirstElevation_deg = edgeFirst_deg(:, 2).';
edgeSecondAzimuth_deg = edgeSecond_deg(:, 1).';
edgeSecondElevation_deg = edgeSecond_deg(:, 2).';

lineMinimumAzimuth_deg = min(firstAzimuth_deg, secondAzimuth_deg);
lineMaximumAzimuth_deg = max(firstAzimuth_deg, secondAzimuth_deg);
lineMinimumElevation_deg = min(firstElevation_deg, secondElevation_deg);
lineMaximumElevation_deg = max(firstElevation_deg, secondElevation_deg);
edgeMinimumAzimuth_deg = min( ...
    edgeFirstAzimuth_deg, edgeSecondAzimuth_deg);
edgeMaximumAzimuth_deg = max( ...
    edgeFirstAzimuth_deg, edgeSecondAzimuth_deg);
edgeMinimumElevation_deg = min( ...
    edgeFirstElevation_deg, edgeSecondElevation_deg);
edgeMaximumElevation_deg = max( ...
    edgeFirstElevation_deg, edgeSecondElevation_deg);
candidatePair = ...
    lineMaximumAzimuth_deg >= edgeMinimumAzimuth_deg - margin_deg & ...
    lineMinimumAzimuth_deg <= edgeMaximumAzimuth_deg + margin_deg & ...
    lineMaximumElevation_deg >= edgeMinimumElevation_deg - margin_deg & ...
    lineMinimumElevation_deg <= edgeMaximumElevation_deg + margin_deg;

lineAzimuthDirection_deg = secondAzimuth_deg - firstAzimuth_deg;
lineElevationDirection_deg = secondElevation_deg - firstElevation_deg;
firstSide = lineAzimuthDirection_deg .* ( ...
    edgeFirstElevation_deg - firstElevation_deg) - ...
    lineElevationDirection_deg .* ( ...
    edgeFirstAzimuth_deg - firstAzimuth_deg);
secondSide = lineAzimuthDirection_deg .* ( ...
    edgeSecondElevation_deg - firstElevation_deg) - ...
    lineElevationDirection_deg .* ( ...
    edgeSecondAzimuth_deg - firstAzimuth_deg);
edgeAzimuthDirection_deg = ...
    edgeSecondAzimuth_deg - edgeFirstAzimuth_deg;
edgeElevationDirection_deg = ...
    edgeSecondElevation_deg - edgeFirstElevation_deg;
lineFirstSide = edgeAzimuthDirection_deg .* ( ...
    firstElevation_deg - edgeFirstElevation_deg) - ...
    edgeElevationDirection_deg .* ( ...
    firstAzimuth_deg - edgeFirstAzimuth_deg);
lineSecondSide = edgeAzimuthDirection_deg .* ( ...
    secondElevation_deg - edgeFirstElevation_deg) - ...
    edgeElevationDirection_deg .* ( ...
    secondAzimuth_deg - edgeFirstAzimuth_deg);
edgeBoundsOverlap = ...
    max(lineMinimumAzimuth_deg, edgeMinimumAzimuth_deg) <= ...
        min(lineMaximumAzimuth_deg, edgeMaximumAzimuth_deg) + 1e-12 & ...
    max(lineMinimumElevation_deg, edgeMinimumElevation_deg) <= ...
        min(lineMaximumElevation_deg, edgeMaximumElevation_deg) + 1e-12;
intersects = candidatePair & edgeBoundsOverlap & ...
    firstSide .* secondSide <= 1e-14 & ...
    lineFirstSide .* lineSecondSide <= 1e-14;
blocked = any(intersects, 2);
if margin_deg <= 0 || all(blocked)
    return;
end

lineLengthSquared_deg2 = max( ...
    lineAzimuthDirection_deg.^2 + lineElevationDirection_deg.^2, eps);
firstFraction = ( ...
    (edgeFirstAzimuth_deg - firstAzimuth_deg) .* ...
        lineAzimuthDirection_deg + ...
    (edgeFirstElevation_deg - firstElevation_deg) .* ...
        lineElevationDirection_deg) ./ lineLengthSquared_deg2;
secondFraction = ( ...
    (edgeSecondAzimuth_deg - firstAzimuth_deg) .* ...
        lineAzimuthDirection_deg + ...
    (edgeSecondElevation_deg - firstElevation_deg) .* ...
...
secondFraction = min(1, max(0, secondFraction));
firstClosestAzimuth_deg = firstAzimuth_deg + ...
    firstFraction .* lineAzimuthDirection_deg;
firstClosestElevation_deg = firstElevation_deg + ...
    firstFraction .* lineElevationDirection_deg;
secondClosestAzimuth_deg = firstAzimuth_deg + ...
    secondFraction .* lineAzimuthDirection_deg;
secondClosestElevation_deg = firstElevation_deg + ...
    secondFraction .* lineElevationDirection_deg;
firstDistanceSquared_deg2 = ...
    (edgeFirstAzimuth_deg - firstClosestAzimuth_deg).^2 + ...
    (edgeFirstElevation_deg - firstClosestElevation_deg).^2;
secondDistanceSquared_deg2 = ...
    (edgeSecondAzimuth_deg - secondClosestAzimuth_deg).^2 + ...
    (edgeSecondElevation_deg - secondClosestElevation_deg).^2;
nearEdge = candidatePair & ( ...
    firstDistanceSquared_deg2 <= margin_deg^2 | ...
    secondDistanceSquared_deg2 <= margin_deg^2);
blocked = blocked | any(nearEdge, 2);
end
    ...
    if ~obstacle.usesSweptEdges
        if obstacle.isUniformTime
            relativeSliceIndex = (queryTime_s - time_s(1)) ./ ...
                obstacle.timeStep_s;
            nearestSliceIndex = round(relativeSliceIndex) + 1;
        else
...
for shiftIndex = 1:numel(azimuthShifts_deg)
    ...
    activeRows = find(~occupied);
    if isempty(activeRows)
        return;
    end
    activeAzimuth_deg = queryImage_deg(activeRows);
    activeElevation_deg = queryElevation_deg(activeRows);
    activeBounds_deg = [min(activeAzimuth_deg), max(activeAzimuth_deg), ...
        min(activeElevation_deg), max(activeElevation_deg)];
    candidateRegion = ...
        slice.bounds_deg(:, 2) >= activeBounds_deg(1) - margin_deg & ...
        slice.bounds_deg(:, 1) <= activeBounds_deg(2) + margin_deg & ...
        slice.bounds_deg(:, 4) >= activeBounds_deg(3) - margin_deg & ...
        slice.bounds_deg(:, 3) <= activeBounds_deg(4) + margin_deg;
    for regionIndex = reshape(find(candidateRegion), 1, [])
...
function [costToGo, diagnostics] = buildDynamicRelaxedCostField( ...
        ...
        safetyMargin_deg, primitiveOffsets, primitiveDurationSteps, ...
        primitiveDistance_deg)
timer = tic;
...
relaxedDurationSteps = unique([1; 2; primitiveDurationSteps(:)]);
...
for durationIndex = 1:numel(relaxedDurationSteps)
    ...
    relaxedEdgeCost = zeros(0, 1);
    if durationSteps <= 2 && azimuthStep_deg > 0
    ...
    if durationSteps <= 2 && elevationStep_deg > 0
    ...
    if durationSteps <= 2
        ...
        if objective == "minimumTime"
            relaxedEdgeCost = duration_s * ones( ...
                size(relaxedDistance_deg));
        else
            relaxedEdgeCost = relaxedDistance_deg;
        end
    end
    macroAtDuration = primitiveDurationSteps == durationSteps;
    relaxedOffsets = [ ...
        relaxedOffsets; primitiveOffsets(macroAtDuration, :)]; %#ok<AGROW>
    if objective == "minimumTime"
        macroEdgeCost = duration_s * ones(nnz(macroAtDuration), 1);
    else
        macroEdgeCost = primitiveDistance_deg(macroAtDuration);
    end
    relaxedEdgeCost = [relaxedEdgeCost; macroEdgeCost]; %#ok<AGROW>
    [relaxedOffsets, ~, offsetGroup] = unique(relaxedOffsets, "rows");
    relaxedEdgeCost = accumarray( ...
        offsetGroup, relaxedEdgeCost, [], @min);
    relaxedOffsetsByDuration{durationIndex} = relaxedOffsets;
    relaxedEdgeCostByDuration{durationIndex} = relaxedEdgeCost;
...
function [costToGo, diagnostics, blocked, ...
        primitiveEdgeCollisionStatus] = buildReverseDijkstraField( ...
        staticObstacleModel, azimuthGrid_deg, elevationGrid_deg, ...
        candidates, primitiveOffsets, primitiveDistance_deg, objective, ...
        maximumVelocity_deg_s, queryTime_s, safetyMargin_deg, allowWrap, ...
        goalConnectionRadius_deg, maximumStaticEdgeChecks)
reverseTimer = tic;
azimuthCount = numel(azimuthGrid_deg);
elevationCount = numel(elevationGrid_deg);
cellCount = azimuthCount * elevationCount;
...
for candidateIndex = 1:numel(candidates)
    ...
    if objective == "minimumTime"
        candidateSeedCost = connectorDistance_deg / ...
            maximumSpeedNorm_deg_s;
    else
        candidateSeedCost = connectorDistance_deg;
    end
    if isempty(staticObstacleModel)
        goalSeedCost(candidateSeedIds) = min( ...
            goalSeedCost(candidateSeedIds), ...
            candidateSeedCost(candidateSeedIds));
        continue;
    end
    ...
    for seedIndex = 1:numel(candidateSeedIds)
        ...
        if staticLineCollisionFree( ...
            ...
            goalSeedCost(cellId) = min( ...
                goalSeedCost(cellId), candidateSeedCost(cellId));
        end
    end
end
goalCellIds = find(isfinite(goalSeedCost));
...
unitDistance_deg = projectionScale * gridStep_deg * ...
    hypot(unitOffsets(:, 1), unitOffsets(:, 2));
relaxedOffsets = [primitiveOffsets; unitOffsets];
relaxedDistance_deg = [primitiveDistance_deg; unitDistance_deg];
if objective == "minimumTime"
    relaxedEdgeCost = relaxedDistance_deg / ...
        hypot(maximumVelocity_deg_s(1), maximumVelocity_deg_s(2));
else
...
relaxedEdgeCost = accumarray( ...
    offsetGroup, relaxedEdgeCost, [], @min);
edgeCount = size(relaxedOffsets, 1);
candidateStaticEdgeChecks = double(cellCount) * double(edgeCount);
exactEdgeFilteringExecuted = ~isempty(staticObstacleModel) && ...
    candidateStaticEdgeChecks <= maximumStaticEdgeChecks;
primitiveEdgeCollisionStatus = zeros( ...
    cellCount, size(primitiveOffsets, 1), "int8");
if exactEdgeFilteringExecuted
    predecessorAllowed = false(cellCount, edgeCount);
    evaluatedStaticEdgeCount = 0;
    collisionRejectedEdgeCount = 0;
    for edgeIndex = 1:edgeCount
        [predecessorCellIds, successorCellIds] = gridOffsetCellPairs( ...
            elevationCount, azimuthCount, ...
            relaxedOffsets(edgeIndex, :), allowWrap);
        edgeIsFree = false(numel(predecessorCellIds), 1);
        endpointsAreFree = ~blocked(predecessorCellIds) & ...
            ~blocked(successorCellIds);
        freeEndpointRow = find(endpointsAreFree);
        edgeIsFree(freeEndpointRow) = staticLinesCollisionFree( ...
            staticObstacleModel, ...
            gridPoints_deg(predecessorCellIds(freeEndpointRow), :), ...
            gridPoints_deg(successorCellIds(freeEndpointRow), :), ...
            safetyMargin_deg, allowWrap);
        predecessorAllowed(predecessorCellIds, edgeIndex) = edgeIsFree;
        evaluatedStaticEdgeCount = evaluatedStaticEdgeCount + ...
            numel(edgeIsFree);
        collisionRejectedEdgeCount = collisionRejectedEdgeCount + ...
            nnz(~edgeIsFree);
    end
    [primitiveOffsetIsPresent, primitiveRelaxedEdgeIndex] = ismember( ...
        primitiveOffsets, relaxedOffsets, "rows");
    for primitiveIndex = 1:size(primitiveOffsets, 1)
        if ~primitiveOffsetIsPresent(primitiveIndex)
            continue;
        end
        [predecessorCellIds, ~] = gridOffsetCellPairs( ...
            elevationCount, azimuthCount, ...
            primitiveOffsets(primitiveIndex, :), allowWrap);
        primitiveIsFree = predecessorAllowed( ...
            predecessorCellIds, ...
            primitiveRelaxedEdgeIndex(primitiveIndex));
        primitiveEdgeCollisionStatus(predecessorCellIds, primitiveIndex) = ...
            int8(2 * primitiveIsFree - 1);
    end
else
...
[costToGo, ~, queueDiagnostics] = computeAzElReverseDijkstra( ...
    ...
    relaxedOffsets, relaxedEdgeCost, allowWrap, predecessorAllowed, ...
    goalSeedCost(goalCellIds));
diagnostics = struct( ...
    "executed", true, ...
    ...
    "relaxedEdgeCount", size(relaxedOffsets, 1), ...
    "candidateStaticEdgeChecks", candidateStaticEdgeChecks, ...
    "evaluatedStaticEdgeCount", evaluatedStaticEdgeCount, ...
    "exactEdgeFilteringExecuted", exactEdgeFilteringExecuted, ...
    "collisionRejectedEdges", collisionRejectedEdgeCount, ...
...
function [predecessorCellIds, successorCellIds] = gridOffsetCellPairs( ...
        elevationCount, azimuthCount, offset, allowWrap)
azimuthOffset = offset(1);
elevationOffset = offset(2);
predecessorElevation = max(1, 1 - elevationOffset): ...
    min(elevationCount, elevationCount - elevationOffset);
successorElevation = predecessorElevation + elevationOffset;
if allowWrap
    predecessorAzimuth = 1:azimuthCount;
    successorAzimuth = mod( ...
        predecessorAzimuth + azimuthOffset - 1, azimuthCount) + 1;
else
    predecessorAzimuth = max(1, 1 - azimuthOffset): ...
        min(azimuthCount, azimuthCount - azimuthOffset);
    successorAzimuth = predecessorAzimuth + azimuthOffset;
end
[predecessorElevationMesh, predecessorAzimuthMesh] = ndgrid( ...
    predecessorElevation, predecessorAzimuth);
[successorElevationMesh, successorAzimuthMesh] = ndgrid( ...
    successorElevation, successorAzimuth);
predecessorCellIds = sub2ind( ...
    [elevationCount azimuthCount], predecessorElevationMesh(:), ...
    predecessorAzimuthMesh(:));
successorCellIds = sub2ind( ...
    [elevationCount azimuthCount], successorElevationMesh(:), ...
    successorAzimuthMesh(:));
end
...
orderingTolerance = 1e-10;
assertion = firstPriority < secondPriority - orderingTolerance || ( ...
    abs(firstPriority - secondPriority) <= orderingTolerance && ( ...
    firstTie < secondTie - orderingTolerance || ( ...
    abs(firstTie - secondTie) <= orderingTolerance && ...
...
for segmentIndex = 1:numel(segments)
    ...
    if ~collisionFree
        [failureStartPosition_deg, ~, ~] = ...
            evaluateConstantJerkLaw(segment.coefficients, 0);
        [failureEndPosition_deg, ~, ~] = ...
            evaluateConstantJerkLaw(segment.coefficients, duration_s);
        validation.message = sprintf( ...
            "Reconstructed %s segment %d on [%.9g, %.9g] s from [%.9g, %.9g] to [%.9g, %.9g] deg intersects an obstacle check.", ...
            string(segment.kind), segmentIndex, ...
            segment.startTime_s, segment.endTime_s, ...
            failureStartPosition_deg(1), failureStartPosition_deg(2), ...
            failureEndPosition_deg(1), failureEndPosition_deg(2));
        return;
    end
...
diagnostics = struct( ...
    ...
    "relaxedEdgeCount", 0, ...
    "candidateStaticEdgeChecks", 0, ...
    "evaluatedStaticEdgeCount", 0, ...
    "exactEdgeFilteringExecuted", false, ...
    "collisionRejectedEdges", 0, ...
    "projectionScale", NaN, ...
    "initialCost", Inf, ...
