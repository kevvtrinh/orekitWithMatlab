...
for k = 1:n
    hasBoundary(k) = sum(isfinite(data.az_deg{k}) & ...
        isfinite(data.el_deg{k})) >= 3;
end
...
shapeCount = 0;
for k = selected(:).'
    shape = boundaryShape(data.az_deg{k}, data.el_deg{k}, ...
...
legendHandles = [sweepHandle firstHandle lastHandle];
...
title(ax, sprintf("%s az/el sweep | %.2f to %.2f s | area %.3f deg^2", ...
    data.targetName, data.time_s(firstIndex), ...
    data.time_s(lastIndex), planarAreaDeg2));
result = struct("SweepShape", sweepShape, "PlanarAreaDeg2", planarAreaDeg2, ...
    "ValidTimeMask", validShape, "ValidTimes_s", data.time_s(validShape), ...
