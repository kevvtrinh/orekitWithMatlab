...
epoch = localEpoch();
data = rectangleObstacle(epoch, 0:40, [-1 1 43 47]);
...
data = obstacleData("Closing gate", epoch + seconds((0:n - 1).'), ...
    azimuth, elevation);
workspace = buildAzElTimeObstacleWorkspace(data);
limits = plannerLimits([40 50]);
options = struct( ...
...
data = obstacleData("Empty", epoch + seconds((0:n - 1).'), empty, empty);
workspace = buildAzElTimeObstacleWorkspace(data);
end

function data = rectangleObstacle(epoch, timeSeconds, bounds)
n = numel(timeSeconds);
azimuth = repmat({ ...
...
data = obstacleData("Rectangle", ...
    epoch + seconds(timeSeconds(:)), azimuth, elevation);
end

function data = obstacleData(name, time, azimuth, elevation)
data = struct( ...
