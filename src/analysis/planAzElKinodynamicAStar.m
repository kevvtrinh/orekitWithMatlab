...
[nodes, startIndex] = appendNode(nodes, start.State, 0, [0 0], 0, ...
...
stoppingTime = max( ...
    max(0, abs(state(3)) - goal.RateToleranceDegPerSec(1)) / ...
    limits.AzimuthAccelerationLimitDegPerSec2, ...
    max(0, abs(state(4)) - goal.RateToleranceDegPerSec(2)) / ...
    limits.ElevationAccelerationLimitDegPerSec2);
value = max(positionTime, stoppingTime);
end

function yes = isGoal(state, goal, limits)
...
yes = state(5) >= goal.EarliestStep && state(5) <= goal.LatestStep && ...
    ...
    abs(state(2) - goal.ElevationDeg) <= goal.PositionToleranceDeg(2) && ...
    abs(state(3)) <= goal.RateToleranceDegPerSec(1) && ...
    abs(state(4)) <= goal.RateToleranceDegPerSec(2);
end
...
function [nodes, index] = appendNode( ...
        nodes, state, parent, acceleration, g, h)
