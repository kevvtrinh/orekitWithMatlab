function result = planAzElKinodynamicARAStar( ...
        obstacleWorkspace, startState, goalState, limits, options)
%PLANAZELKINODYNAMICARASTAR Anytime rate/acceleration-limited slew planner.
%
% result = planAzElKinodynamicARAStar( ...
%     workspace, start, goal, limits, options)
%
% ARA* searches the finite state lattice [azimuth, elevation, azimuth
% rate, elevation rate, time] with constant-acceleration motion
% primitives. It reuses OPEN, CLOSED, INCONS, and state costs while
% reducing epsilon. Each completed iteration certifies
%
%   returned cost <= epsilon * optimal lattice cost.
%
% This is a lattice guarantee, not a continuous-space guarantee.
%
...
if pointCollision(obstacleWorkspace, start.State, ...
    ...
    error("planAzElKinodynamicARAStar:StartInCollision", ...
...
goalIndex = 0;
if isGoal(start.State, start.Acceleration, goal, limits)
    goalIndex = startIndex;
else
    openNode(startKey) = startIndex;
...
expanded = 0;
generated = 1;
history = zeros(numel(options.EpsilonSchedule), 11);
historyCount = 0;
certifiedBound = Inf;
lastEpsilon = options.EpsilonSchedule(1);
terminationReason = "epsilonScheduleComplete";
searchTimer = tic;
for iteration = 1:numel(options.EpsilonSchedule)
    epsilon = options.EpsilonSchedule(iteration);
    lastEpsilon = epsilon;
    if iteration > 1
        [heap, openNode] = rebuildOpen( ...
            nodes, bestNode, openNode, inconsNode, epsilon, options);
        inconsNode = nodeMap();
        closed = logicalMap();
    end

    expandedBefore = expanded;
    [nodes, bestNode, heap, openNode, inconsNode, closed, ...
        goalIndex, expanded, generated, completed, stopReason] = ...
        improvePath(nodes, bestNode, heap, openNode, inconsNode, ...
        closed, goalIndex, expanded, generated, controls, ...
        obstacleWorkspace, start, goal, limits, options, ...
        epsilon, searchTimer);

    if goalIndex ~= 0
        if completed
            certifiedBound = frontierBound( ...
                nodes, bestNode, openNode, inconsNode, ...
                goalIndex, epsilon);
        end
        nodePath = reconstructNodePath(nodes, goalIndex);
        currentPath = makePathTable(nodes, nodePath, ...
            start.TimeSeconds, referenceTime, limits, options);
        historyCount = historyCount + 1;
        history(historyCount, :) = [ ...
            iteration, epsilon, double(completed), certifiedBound, ...
            nodes.G(goalIndex), currentPath.ElapsedSeconds(end), ...
            pathAngularLength(currentPath, options), ...
            expanded - expandedBefore, expanded, generated, ...
            toc(searchTimer)];
    end
    ...
    if goalIndex == 0 && completed
        terminationReason = "noPath";
        break;
    end
    if completed && epsilon <= 1 + 1e-12
        certifiedBound = 1;
        terminationReason = "optimal";
        history(historyCount, 4) = 1;
        break;
    end
end
...
if goalIndex == 0
    switch terminationReason
        case "wallTimeLimit"
            message = ...
                "Search reached MaxWallTimeSeconds before finding a path.";
        case "expansionLimit"
    ...
    result.SearchElapsedSeconds = toc(searchTimer);
    result.TerminationReason = terminationReason;
    result.SolutionHistory = makeHistoryTable(history, historyCount);
    return;
end
...
result = struct();
result.Success = true;
result.Algorithm = "ARA*";
result.OptimalOnLattice = certifiedBound <= 1 + 1e-12;
if result.OptimalOnLattice
    result.Message = "Optimal " + options.Objective + ...
        " lattice path certified by ARA*.";
elseif isfinite(certifiedBound)
    result.Message = sprintf( ...
        "ARA* path found with a certified %.6gx lattice bound.", ...
        certifiedBound);
else
    result.Message = ...
        "ARA* found an incumbent before completing a bounded iteration.";
end
result.Objective = options.Objective;
result.HeuristicWeight = lastEpsilon;
result.RequestedEpsilon = lastEpsilon;
result.SuboptimalityBound = certifiedBound;
...
result.SearchElapsedSeconds = toc(searchTimer);
result.TerminationReason = terminationReason;
result.IterationCount = historyCount;
result.SolutionHistory = makeHistoryTable(history, historyCount);
result.Path = path;
...
function [nodes, bestNode, heap, openNode, inconsNode, closed, ...
        goalIndex, expanded, generated, completed, stopReason] = ...
        improvePath(nodes, bestNode, heap, openNode, inconsNode, ...
        closed, goalIndex, expanded, generated, controls, workspace, ...
        start, goal, limits, options, epsilon, searchTimer)
completed = false;
stopReason = "";
while true
    [heap, openNode, currentIndex, minimumKey] = ...
        peekOpen(heap, openNode, bestNode, nodes, limits, options);
    if goalIndex ~= 0 && ...
            (currentIndex == 0 || nodes.G(goalIndex) <= minimumKey + 1e-12)
        completed = true;
        return;
    end
    if currentIndex == 0
    ...
    [heap, poppedIndex] = heapPop(heap);
    if poppedIndex ~= currentIndex
        error("planAzElKinodynamicARAStar:InvalidOpenQueue", ...
            "The ARA* OPEN queue lost its minimum-key node.");
    end
    current = nodes.State(currentIndex, :);
    currentAcceleration = nodes.Acceleration(currentIndex, :);
    currentKey = stateKey( ...
        current, currentAcceleration, limits, options);
    remove(openNode, currentKey);
    closed(currentKey) = true;
    expanded = expanded + 1;
    if current(5) >= goal.LatestStep || ...
            ~canStillReach(current, goal, limits, options)
        continue;
    end
    ...
    collision = primitiveCollisions(workspace, current, ...
    ...
    for candidateIndex = 1:numel(candidateControls)
        ...
        elseif isKey(closed, nextKey)
            inconsNode(nextKey) = nextIndex;
        else
            openNode(nextKey) = nextIndex;
            heap = heapPush(heap, nextIndex, ...
                tentativeG + epsilon * h, tie);
        end
    end
end
end

function [heap, openNode, nodeIndex, minimumKey] = ...
        peekOpen(heap, openNode, bestNode, nodes, limits, options)
nodeIndex = 0;
minimumKey = Inf;
while heap.Count > 0
    candidate = heap.Node(1);
    key = stateKey(nodes.State(candidate, :), ...
        nodes.Acceleration(candidate, :), limits, options);
    valid = isKey(openNode, key) && ...
        openNode(key) == candidate && ...
        isKey(bestNode, key) && bestNode(key) == candidate;
    if valid
        nodeIndex = candidate;
        minimumKey = heap.F(1);
        return;
...
function [heap, openNode] = rebuildOpen( ...
        nodes, bestNode, openNode, inconsNode, epsilon, options)
candidate = nodeMap();
openKeys = keys(openNode);
for k = 1:numel(openKeys)
    key = openKeys{k};
    if isKey(bestNode, key)
        candidate(key) = bestNode(key);
    end
end
inconsKeys = keys(inconsNode);
for k = 1:numel(inconsKeys)
    key = inconsKeys{k};
    if isKey(bestNode, key)
        candidate(key) = bestNode(key);
    end
...
openNode = nodeMap();
heap = emptyHeap(max(options.InitialNodeCapacity, candidate.Count));
candidateKeys = keys(candidate);
for k = 1:numel(candidateKeys)
    key = candidateKeys{k};
    nodeIndex = candidate(key);
    openNode(key) = nodeIndex;
    priority = nodes.G(nodeIndex) + epsilon * nodes.H(nodeIndex);
    heap = heapPush( ...
        heap, nodeIndex, priority, nodes.Tie(nodeIndex));
end
end

function bound = frontierBound( ...
        nodes, bestNode, openNode, inconsNode, goalIndex, epsilon)
incumbentCost = nodes.G(goalIndex);
if incumbentCost <= 1e-12
    bound = 1;
    return;
end
lowerBound = Inf;
frontiers = {openNode, inconsNode};
for frontierIndex = 1:numel(frontiers)
    frontier = frontiers{frontierIndex};
    frontierKeys = keys(frontier);
    for k = 1:numel(frontierKeys)
        key = frontierKeys{k};
        if ~isKey(bestNode, key)
            continue;
        end
        nodeIndex = bestNode(key);
        lowerBound = min( ...
            lowerBound, nodes.G(nodeIndex) + nodes.H(nodeIndex));
    end
end
if ~isfinite(lowerBound)
    bound = 1;
else
    bound = max(1, min(epsilon, incumbentCost / ...
        max(lowerBound, eps(incumbentCost))));
end
end

function map = nodeMap()
map = containers.Map('KeyType', 'char', 'ValueType', 'double');
end

function map = logicalMap()
map = containers.Map('KeyType', 'char', 'ValueType', 'logical');
end

function historyTable = makeHistoryTable(history, count)
history = history(1:count, :);
historyTable = array2table(history, 'VariableNames', { ...
    'Iteration', 'RequestedEpsilon', 'Completed', ...
    'CertifiedSuboptimalityBound', 'ObjectiveCost', ...
    'CostSeconds', 'AngularPathLengthDeg', ...
    'ExpandedThisIteration', 'ExpandedTotal', ...
    'GeneratedTotal', 'ElapsedSeconds'});
historyTable.Completed = logical(historyTable.Completed);
end
...
defaults = struct( ...
    ...
    "EpsilonSchedule", [2.5 2 1.5 1.25 1], ...
...
validateattributes(options.EpsilonSchedule, {'numeric'}, ...
    {'vector', 'nonempty', 'real', 'finite', '>=', 1});
options.EpsilonSchedule = double(options.EpsilonSchedule(:).');
if any(diff(options.EpsilonSchedule) > 1e-12)
    error("planAzElKinodynamicARAStar:InvalidEpsilonSchedule", ...
...
if options.AzimuthWrap ~= 0 && options.AzimuthWrap ~= 1
    error("planAzElKinodynamicARAStar:InvalidAzimuthWrap", ...
...
if ~any(mode == ["polygon", "bounds"])
    error("planAzElKinodynamicARAStar:InvalidCollisionMode", ...
...
if ~isscalar(value)
    error("planAzElKinodynamicARAStar:InvalidObjective", ...
...
if any(abs(checks - round(checks)) > 1e-8)
    error("planAzElKinodynamicARAStar:IncompatibleLattice", ...
...
if options.AzimuthWrap
    ...
    if abs(binCount - round(binCount)) > 1e-8
        error("planAzElKinodynamicARAStar:IncompatibleAzimuthWrap", ...
...
if abs(start.Acceleration(1)) > ...
    ...
    error("planAzElKinodynamicARAStar:StateOutsideLimits", ...
...
if latest < earliest || earliest < startSeconds - 1e-9
    error("planAzElKinodynamicARAStar:InvalidGoalWindow", ...
...
if goal.ElevationDeg < limits.ElevationLimitsDeg(1) || ...
    ...
    error("planAzElKinodynamicARAStar:GoalOutsideLimits", ...
...
integratedLength = ...
    (dt / (sampleCount - 1)) / 3 * sum(weights .* speed);
displacement = rate * dt + 0.5 * acceleration * dt^2;
lengthDeg = max(integratedLength, hypot( ...
...
function [nodes, index] = appendNode( ...
        nodes, state, parent, acceleration, g, h, tie, epsilon)
if nodes.Count >= size(nodes.State, 1)
    ...
    nodes.H(oldCapacity + 1:newCapacity, 1) = inf;
...
result = struct( ...
    ...
    "HeuristicWeight", options.EpsilonSchedule(1), ...
    "RequestedEpsilon", options.EpsilonSchedule(1), ...
    ...
    "TerminationReason", "", ...
    "IterationCount", 0, ...
    "SolutionHistory", makeHistoryTable(zeros(0, 11), 0), ...
    "Path", path, ...
...
elseif isnumeric(value) && isscalar(value) && isfinite(value)
    ...
    error("planAzElKinodynamicARAStar:InvalidTime", ...
...
if ~isstruct(input) || ~all(isfield(input, cellstr(names)))
    error("planAzElKinodynamicARAStar:MissingField", ...
...
if ~isstruct(workspace) || ~isfield(workspace, "Format") || ...
    ...
    error("planAzElKinodynamicARAStar:InvalidWorkspace", ...
