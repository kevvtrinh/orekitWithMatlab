...
for w = 1:windowCount
    ...
    samples = cell(numel(projection.time_s), 1);
    for sampleIndex = 1:numel(projection.time_s)
        [commandAzimuthDeg, commandElevationDeg] = azElCenter( ...
            projection.az_deg{sampleIndex}, projection.el_deg{sampleIndex});
        commandInsideFor = isfinite(commandAzimuthDeg) && ...
            isfinite(commandElevationDeg) && ...
            commandElevationDeg >= max(0, 90 - sensor.FieldOfRegardDeg) && ...
            commandElevationDeg <= 90;
        samples{sampleIndex} = struct( ...
            "tOffsetSec", round(projection.time_s(sampleIndex), 3), ...
            "boundarySegments", {finiteAzElSegments( ...
                projection.az_deg{sampleIndex}, ...
                projection.el_deg{sampleIndex})}, ...
            "commandAzimuthDeg", round(commandAzimuthDeg, 5), ...
            "commandElevationDeg", round(commandElevationDeg, 5), ...
            "commandInsideFor", commandInsideFor, ...
...
function [azimuthDeg, elevationDeg] = azElCenter(azimuth, elevation)
...
azimuthDeg = atan2d(mean(sind(azimuth(valid))), ...
    mean(cosd(azimuth(valid))));
elevationDeg = mean(elevation(valid));
end
