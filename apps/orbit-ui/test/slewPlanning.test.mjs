...
const options = { platform: "Slew Demo", from: "South Target", to: "North Station", obstacle: "Vietnam keep-out", startSec: 0, durationSec: 30 };

test("Earth region export moves with the satellite and a direct slew crosses it", () => {
  const request = makeSlewRequest(avoidanceScene(), options);
  ...
  let collisions = 0;
  request.obstacles[0].slices.forEach((slice, i) => {
...
test("rejects hidden Earth obstacles, invalid time spans and unsafe bridge inputs", () => {
  ...
  scene.areaOutlines[0].centerLonDeg -= 180;
  assert.throws(()=>makeSlewRequest(scene,options),/horizon/);
});

test("actual MATLAB detour clears the moving Earth region and stays under the sensor rate", () => {
  // Generated by planOrbitUiSlew / progressiveSafeIntervalAStar for avoidanceScene.
  const scene = avoidanceScene(), request = makeSlewRequest(scene,options);
...
test("vendored main BMTP result passes independent motion validation and the moving Earth check", () => {
  const scene=avoidanceScene(), request=makeSlewRequest(scene,options);
  ...
  assert.equal(result.plannerRevision,"b22e7bb5cd7e2e6028822a5bd8b4d55a49e71ddc");
  ...
  const checked=validateSlewResult(request,result,scene);
  assert.equal(checked.groundTrace.length,625); assert.ok(checked.groundTrace.every(Boolean));
